...
const colorToHex = (color: [number, number, number] | null) => {
  if (color === null) {
    return null;
  }
  
  const r = Math.round(color[0] * 255).toString(16);
  const g = Math.round(color[1] * 255).toString(16);
  const b = Math.round(color[2] * 255).toString(16);
  return `#${r}${g}${b}`;
}
...
const groupedParameters = computed(() => {
  ...
  props.parameterDefinitions.forEach(param => {
    if (param.type === 'group') {
...
const params = reactive(
  props.parameterDefinitions.filter(
    ...
    if (param.type === 'color') {
      const hex = props.params[(param as ValueParameter).name] ? colorToHex(props.params[(param as ValueParameter).name] as [number, number, number] | null) : (param as ValueParameter).initial || null;
      acc[(param as ValueParameter).name] = hex;
    } else {
      acc[(param as ValueParameter).name] = props.params[(param as ValueParameter).name] || (param as ValueParameter).initial || null;
    }
    return acc;
...
const fixColorParams = (originalParams: Record<string, unknown>) => {
  ...
  return props.parameterDefinitions.filter(
    ({type}) => type !== 'group'
  ).reduce((acc, param) => {
    if (param.type === 'color') {
